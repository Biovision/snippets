...
RSpec.describe BrandsController, type: :controller do
  ...
  shared_examples 'forbidden_editing' do
    ...
    it 'redirects to entity administration page' do
      expect(response).to redirect_to(admin_brand_path(entity))
    end
  end
  ...
  describe 'post create' do
    context 'when parameters are valid' do
      let(:action) { -> { post :create, params: { brand: attributes_for(:brand) } } }
      ...
      context 'authorization and redirects' do
        ...
        it 'redirects to created entity' do
          expect(response).to redirect_to(admin_brand_path(entity.class.last))
        end
      end
    end

    context 'when parameters are invalid' do
      let(:action) { -> { post :create, params: { brand: { name: ' ' } } } }
  ...
  describe 'patch update' do
    let(:action) { -> { patch :update, params: { id: entity, brand: { name: 'Changed' } } } }
    ...
    context 'when parameters are invalid' do
      before :each do
        patch :update, params: { id: entity, brand: { name: ' ' } }
      end
